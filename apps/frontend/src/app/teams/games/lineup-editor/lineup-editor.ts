...
export class LineupEditor implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (gameId && teamId) {
      void this.loadData(teamId, gameId);
    }
  }

  private async loadData(teamId: string, gameId: string): Promise<void> {
    this.isLoading.set(true);
    this.errorMessage.set(null);
    try {
      const [game, players, lineup] = await Promise.all([
        firstValueFrom(this.gamesService.getGame(gameId)),
